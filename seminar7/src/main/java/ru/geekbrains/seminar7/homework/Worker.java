...
public class Worker extends JobSeeker implements Observer{

    public Worker(String name, String surName, Professions specialization, int salary, int experienceYears) {
        super(name,surName,specialization,salary,experienceYears);
    }

    public Worker(String name, String surName, Professions specialization, int salary, int experienceYears, String backgroundDescription) {
    ...
    public boolean receiveOffer(Vacancy vacancy) {
        if (this.salary <= vacancy.getSalary()){
            ...
            System.out.printf("Рабочий: %s %s. Я не готов работать за такую зп! (Компания: %s; Заработная плата: %d)\n",name,surName, vacancy.getFromCompany().getBrandName(),salary);
        }
        return false;
    }
